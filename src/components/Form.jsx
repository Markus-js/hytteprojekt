...
export const Form = () => {
  const { hytter, reservationData, setSuccess } = useContext(AppContext);

  // Destrutured prop from => reservationData
  const { price, number, type } = reservationData;
      ...
      <form className={Style.form} onSubmit={sendEmail}>
        ...
        <div style={{ display: "none" }}>
          <input
            ...
            onChange={(e) =>
              setData({ ...data, txt_after_reservation: e.target.value })
            }
          />
          <input
            name="num"
            value={number}
            onChange={(e) =>
              setData({ ...data, reservation_number: e.target.value })
            }
          />
          <input
            name="type"
