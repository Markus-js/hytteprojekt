...
const ModalChildReservation = () => {
  ...
  const { txt_after_reservation } = hytter;

  // Destrutured props from => selectedHytteDataByID
  const { description, number, type } = reservationData;
  ...
  return (
    <main>
      <header className={success ? "header header--green" : "header"}>
        {!success && (
          <h2>
            Du er nu ved at reservere {type} nr. {number}
          </h2>
        )}
        {success && (
          <h2>
            Du har reserveret {type} nr. {number}
          </h2>
        )}
      </header>
      <div
        className={
          success
            ? Style.content_wrapper && Style.content_wrapper_centered
      ...
      >
        ...
        {success && !initializeCount && (
          <button className="btn btn--green" onClick={() => setRunTimer(true)}>
            Luk
          </button>
        )}
      </div>
